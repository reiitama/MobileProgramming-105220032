...
class MainActivity : AppCompatActivity() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.menu_item1 -> {
                tvSelectedOption.text = "Pilih Opsi: Opsi 1"
                return true
            }
            R.id.menu_item2 -> {
                tvSelectedOption.text = "Pilih Opsi: Opsi 2"
                return true
            }
            R.id.menu_item3 -> {
                tvSelectedOption.text = "Pilih Opsi: Opsi 3"
                return true
            }
    ...
    private fun showPicker() {
        ...
        builder.setItems(pickerOptions) { _, which ->
            tvSelectedOption.text = "Opsi yang dipilih: ${pickerOptions[which]}"
        }
